...
from continual_world.methods.regularization import EWCHelper, L2Helper, MASHelper
...
from continual_world.utils.utils import get_activation_from_str
from continual_world.sequoia_integration.wrappers import wrap_sequoia_env, concat_x_and_t
...
class SACMethod(Method, target_setting=DiscreteTaskAgnosticRLSetting):  # type: ignore
    family: ClassVar[str] = "continual_world"

    def __init__(self, algo_config: AlgoConfig = None):
        super().__init__()
        self.algo_config = algo_config or AlgoConfig()

        self.task_config: TaskConfig
        self.actor_cl: Type[tf.keras.Model]
        self.critic_cl: Type[tf.keras.Model]
    ...
    def configure(self, setting: DiscreteTaskAgnosticRLSetting) -> None:
        ...
        args_for_epoch_logger = dict(**asdict(self.task_config), **asdict(self.algo_config))
        self.logger = EpochLogger(self.task_config.logger_output, config=args_for_epoch_logger)
        # NOTE: run_cl.py did this, but I don't understand why! (Not doing it, personally.)
        # del self.task_config.logger_output
        ...
        self.actor = self.actor_cl(**actor_kwargs)
        ...
        if cl_method == "packnet":
            ...
            episodic_mem_size = self.algo_config.episodic_mem_per_task * nb_tasks
        ...
        if self.algo_config.alpha == "auto":
            ...
            self.all_log_alpha = tf.Variable(
                np.ones((num_tasks, 1), dtype=np.float32), trainable=True
            )
            if self.algo_config.target_output_std is None:
                ...
                self.target_entropy = (
                    np.prod(setting.action_space.shape).astype(np.float32) * target_1d_entropy
                )
    ...
    def get_actions(
        ...
        assert False, (action, action_space)
        return action_space.dtype(action)
            ...
            for seq_idx, test_env in enumerate(test_envs):
                ...
                for j in range(num_eps):
                    ...
                    while not (d or (ep_len == self.task_config.max_ep_len)):
                        ...
                        ep_len += 1
                    self.logger.store(
                        {key_prefix + "return": ep_ret, key_prefix + "ep_length": ep_len}
                    )
